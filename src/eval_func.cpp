    ...
    void *abc_data)
{
    ...
    {
        ratio = datTime / weibull_lambdas.col(ll);
        // ratio = arma::min(ratio, arma::vec(mydata_parm->N).fill(upperbound)); // numerical stability
    ...
    void *abc_data)
{
    ...
    for(unsigned int ll=0; ll<(mydata_parm->L); ++ll)
    {
        ...
        tmp = alphas.col(ll) / alphaRowsum_tmp %  weibullS.col(ll);
...
)
{
    ...
    {
        mu_l_tmp += datX.col(mydata_parm->jj - 1) * delta;
    }
...
)
{
    ...
    arma::vec alphaRowsum_candidate =
        alphaRowsum_base - alphas_base.col(l) + alpha_l_candidate;
    ...
    logpost_first = arma::max(logpost_first, arma::vec(N).fill(lowerbound)); 
